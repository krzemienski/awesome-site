...
- Admin tab APIs: All 10 endpoints verified. /api/admin/enrichment is POST-only (triggers jobs), GET is at /api/admin/enrichment/jobs. /api/admin/export/validate is POST-only (needs markdown body). All return real data with success:true.
- Validation scripts created: validate-login.sh, validate-export.sh, validate-cors.sh, validate-api-versioning.sh in scripts/ directory
