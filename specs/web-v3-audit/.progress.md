...
- [x] 1.5 Create VariationSwitcher floating UI - pending_commit
...
- [x] 2.8 Upgrade AdminSidebar to shadcn with 20-tab collapsible groups
...
- [x] 7.1 Add AI Recommendations panel to resource pages - pending_commit
...
- (Implementation) Variation A CSS tokens expanded from 11 layout-only tokens to 6 groups: layout (11), shadow (6, more dramatic than B - 0.15 opacity, 12px blur, 6px offset), sidebar spacing (2), card (1, radius 0px), typography (2, -0.02em letter-spacing, 1.7 line-height), interaction states (4). No color tokens - stays CSS-only layout/spacing.
...
- (Implementation) Admin sidebar rewritten from flat 14-item Button list to 6 collapsible groups with 20 items using shadcn Sidebar primitives + Collapsible. Groups auto-expand when active tab is inside them. NAV_ITEMS export replaced with NAV_GROUPS export. Only admin-layout.tsx imports AdminSidebar (no other consumers of old exports).
- (Implementation) AdminLayoutWrapper rewritten from custom Sheet+hidden div pattern to SidebarProvider+Sidebar+SidebarInset. Removed useState, Sheet imports, hamburger button. AdminSidebar renders fragments (<>SidebarHeader+SidebarContent+SidebarFooter</>) so wrapping in <Sidebar> works directly. SidebarProvider handles mobile drawer, tablet icon-only, desktop expanded automatically.
- (Implementation) ValidationTab uses two-step flow: first fetches markdown via POST /api/admin/export, then validates via POST /api/admin/export/validate. LintResult has {valid, errors: [{line, rule, message}], warnings: [{line, rule, message}]}. Reuses existing DataTable component with toolbarContent prop for filter dropdown.
- (Implementation) ResearchTab uses ResizablePanelGroup from react-resizable-panels. The prop is `orientation` (not `direction`) for horizontal/vertical layout. ResearchJob has `_count.findings` from Prisma include. Job types: validation, enrichment, discovery, trend_analysis, comprehensive. Status reuses EnrichmentJobStatus enum (pending/processing/completed/failed/cancelled).
- (Implementation) LinkHealthTab uses GET /api/admin/link-health for cached results and POST to trigger new check. LinkCheckResult has statusCode (nullable), responseTime (ms), error (nullable), healthy (boolean). Status derived from data: timeout (error contains "timed out"), redirect (3xx code), healthy/broken. Filter dropdown is client-side. Disable button calls PATCH /api/admin/resources/:id with {status: "disabled"}.
- (Implementation) Variation B CSS has clean separation: layout tokens (sidebar-width, card-padding, etc.) + shadow tokens (from shadcn cyberpunk get_theme) + sidebar/card spacing refinements. NO color tokens - those stay in globals.css [data-theme] selectors. Shadow values: hsl(0 0% 0%) color, 0.1 opacity, 8px blur, -2px spread, 0/4px offset. Card radius 0px to match cyberpunk aesthetic.
- (Implementation) In Next.js 16 with Turbopack, `next/dynamic` with `ssr: false` is NOT allowed in Server Components. Client components with `"use client"` directive can be directly imported into Server Components - they auto-create a client boundary. No need for dynamic import.
- (Implementation) Variation C CSS expanded from 11 layout tokens to 6 groups: layout (11), shadow (6, neon glow - hsl(320 100% 50%) color, 0.2 opacity, 16px blur), sidebar spacing (2), card (1, radius 0px), typography (2, -0.01em letter-spacing, 1.6 line-height), interaction states (4), plus Stitch-specific glow tokens (glow-spread 4px, glow-opacity 0.3). Key differentiators from A/B: smaller sidebar (260px), thicker borders (2px), taller nav items (44px), larger sidebar header (72px), slower transitions (250ms), extra bold headings (800).
