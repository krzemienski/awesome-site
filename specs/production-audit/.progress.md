...
- [x] 2.5 [VERIFY] Phase 2 midpoint checkpoint - typecheck+lint+build all PASS
- [x] 2.6 Add Vercel Analytics and Speed Insights
- [x] 2.7 Validate social auth provider configuration
- [x] 2.8 Complete .env.example documentation - a29b438
- [x] 2.9 [VERIFY] Phase 2 complete checkpoint - all 9 deliverables verified, typecheck+lint+build PASS
...
- [FTS] `buildResourceWhere` became async after adding `$queryRaw` for FTS. `resource-service.ts` caller must `await` it. Any new callers must also await.
- [FTS] Prisma `Unsupported("tsvector")?` field is nullable and excluded from Prisma Client type generation -- all FTS access must use `$queryRaw`.
- [FTS] FTS search strategy: fetch matching IDs via raw `@@` query, then use `{ id: { in: ftsIds } }` in Prisma where clause to combine with other ORM filters. URL LIKE kept as OR fallback.
...
### Verification: 2.5 [VERIFY] Phase 2 midpoint checkpoint
- Status: PASS
- Quality Commands:
  - `npm run typecheck` (tsc --noEmit): PASS (exit 0, zero errors)
  - `npm run lint` (eslint): PASS (exit 0, zero errors)
  - `npm run build` (next build): PASS (exit 0)
    ...
    - TypeScript check passed
    - Generated 76/76 static pages in 268.1ms
    - All routes compiled (107 app routes listed)
    - Warnings (non-blocking): Sentry deprecation notices, Better Auth social provider env vars missing, metadataBase not set, middleware-to-proxy deprecation
- No fixes needed -- all three commands passed clean
- Phase 2 midpoint deliverables verified: CSP nonce (2.1), Upstash rate limiting (2.2), FTS with tsvector+GIN (2.3), privacy+terms pages (2.4)

## Next

Task 3.1 Lazy-load ReactQueryDevtools in dev only

### Verification: 2.9 [VERIFY] Phase 2 complete checkpoint
- Status: PASS
- Quality Commands:
  - `npm run typecheck` (tsc --noEmit): PASS (exit 0, zero errors)
  - `npm run lint` (eslint): PASS (exit 0, zero errors)
  - `npm run build` (next build): PASS (exit 0)
    ...
    - TypeScript check passed
    - Generated 76/76 static pages in 282.6ms
    - All routes compiled (107+ app routes listed)
    - Warnings (non-blocking): Sentry deprecation notices, Better Auth social provider env vars missing, metadataBase not set, middleware-to-proxy deprecation
- Phase 2 Complete Deliverables Verified:
  - CSP nonce in middleware.ts: PRESENT (generateNonce, buildCspHeader, applyCspHeaders with nonce-based script-src)
  - Upstash rate limiting in rate-limit.ts: PRESENT (@upstash/ratelimit sliding window, in-memory fallback)
  - FTS with tsvector + GIN index: PRESENT (schema Unsupported("tsvector"), migration 0002 with GIN index + trigger, resource-queries.ts uses @@ operator)
  - Privacy policy page: PRESENT (src/app/(public)/privacy/page.tsx)
  - Terms of service page: PRESENT (src/app/(public)/terms/page.tsx)
  - Vercel Analytics in layout.tsx: PRESENT (@vercel/analytics/react)
  - SpeedInsights in layout.tsx: PRESENT (@vercel/speed-insights/next)
  - Conditional OAuth providers in auth.ts: PRESENT (socialProviders built conditionally, warnings logged)
  - Complete .env.example: PRESENT (139 lines, all env vars documented with categories and annotations)
